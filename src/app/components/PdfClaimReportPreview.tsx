...
export default function PdfClaimReportPreview({ car, draft }: Props) {
  if (!car || !draft) {
    return (
    ...
    <div className="mx-auto max-w-6xl bg-white rounded-2xl shadow-lg p-6">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6 text-black">
        ...
        <div className="bg-zinc-50 rounded-lg p-4 space-y-3">
          ...
          <p className="text-sm">
            <span className="font-medium">สถานที่:</span> {draft.province}{" "}
            {draft.district} {draft.road}
          </p>
          <p className="text-sm">
          ...
          {draft.details && (
            <p className="text-sm">
              <span className="font-medium">รายละเอียด:</span> {draft.details}
            </p>
          )}
        </div>
